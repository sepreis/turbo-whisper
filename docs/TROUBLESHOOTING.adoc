...
|**Windows: Hotkey not working or conflicts with system**
|Many hotkey combinations are reserved on Windows: Alt+Space (window menu), Ctrl+Shift+Space (input method), Ctrl+Alt+Space (various apps), Win+Shift+V (clipboard history). The default on Windows is F8. If this conflicts, try F9-F12 or Ctrl+Shift+M. Edit `hotkey` in `%APPDATA%\turbo-whisper\config.json`.
