...
|Some terminals may freeze when receiving rapid keyboard input via evdev. Turbo Whisper types at 5ms per character, which can overwhelm some terminals with long text. Workaround: keep dictations shorter, or disable `auto_paste` and use Ctrl+V to paste from clipboard instead.
|===
